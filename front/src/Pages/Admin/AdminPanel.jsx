...
const AdminPanel = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(`http://localhost:3000/events?page=${page}&limit=10`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (eventForm._id) {
        await axios.put(`http://localhost:3000/events/${eventForm._id}`, payload);
      } else {
        await axios.post('http://localhost:3000/events', payload);
      }

      const response = await axios.get(`http://localhost:3000/events?page=1&limit=${page * 10}`);
