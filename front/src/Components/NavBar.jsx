  ...
  return (
    <nav className="bg-gradient-to-r from-indigo-900 to-purple-800 text-white">
      <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
        <Link to={user?.role === 'admin' ? "/admin" : "/"} className="text-3xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-yellow-400 to-orange-500">
        ...
        <div className="hidden md:flex space-x-8 items-center">
          {user?.role !== 'admin' && (
            <div className="flex items-center space-x-6 bg-white/10 backdrop-blur-sm px-6 py-2 rounded-full">
              <Link to="/" className="hover:text-yellow-300 flex items-center gap-1 transition-all">
                <Home size={16} /> Home
              </Link>
              <Link to="/events" className="hover:text-yellow-300 flex items-center gap-1 transition-all">
                <Calendar size={16} /> Events
              </Link>
              <Link to="/contact" className="hover:text-yellow-300 flex items-center gap-1 transition-all">
          ...
          {!isAuthenticated ? (
            <Link to="/login" 
              ...
              hover:from-yellow-600 hover:to-orange-600 shadow-lg hover:shadow-yellow-500/30 transition-all flex items-center gap-2">
              <User size={16} /> Login
            </Link>
          ) : (
          ...
          <div className="pt-2">
            {!isAuthenticated ? (
              <Link to="/login" onClick={() => setIsOpen(false)}
                className="w-full text-center bg-gradient-to-r from-yellow-500 to-orange-500 text-white px-6 py-3 rounded-lg 
                hover:from-yellow-600 hover:to-orange-600 shadow-lg transition-all flex justify-center items-center gap-2">
                <User size={18} /> Login
              </Link>
            ) : (
